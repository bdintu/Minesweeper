...
void DrawX(int Xpos,int Ypos,uint16_t color)
{
	...
	for (int i=0 ; i<7 ;i++){
		//LCD_DrawFullRect(Xpos+(i*4), Ypos+(i*4), 4, 4);
	  LCD_DrawFullCircle(Xpos+(i*4)+2, Ypos+(i*4)-2,2);}
	Xpos = tmpx;
	Ypos = tmpy+(4*7);
	for (int i=0 ; i<7 ;i++){
	  //LCD_DrawFullRect(Xpos+(i*4), Ypos-(i*4), 4, 4);
...
if(numShip==3){
	LCD_SetBackColor(Pink2);
	LCD_SetTextColor(Pink);
}
if(numShip==4){
	LCD_SetBackColor(Orange2);
	LCD_SetTextColor(Orange);
}
if(numShip==5){
	LCD_SetBackColor(Purple2);
	LCD_SetTextColor(Purple);
}
if(numShip==6){
	LCD_SetBackColor(Blue3);
	LCD_SetTextColor(Blue4);
}
	
  LCD_DrawFullRect(Xpos+4,Ypos-4,32, 32);
...
{
	for(int i=0; i<3; i++){
	LCD_DrawLine(Xpos+i, Ypos-i, Width-i, Horizontal);
  LCD_DrawLine((Xpos + Height)-i, Ypos-i, Width-i, Horizontal);
  
  LCD_DrawLine(Xpos+i, Ypos-i, Height-i, Vertical);
  LCD_DrawLine(Xpos+i, (Ypos - Width +1)+i, Height-i, Vertical);
	}	
}
...
void LCD_DrawFullCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
   ...
   while (CurX <= CurY)
   {
     if(CurY > 0) 
     {
       LCD_DrawLine(Xpos - CurX, Ypos + CurY, 2*CurY, Horizontal);
       LCD_DrawLine(Xpos + CurX, Ypos + CurY, 2*CurY, Horizontal);
     }
 
     if(CurX > 0) 
     {
       LCD_DrawLine(Xpos - CurY, Ypos + CurX, 2*CurX, Horizontal);
       LCD_DrawLine(Xpos + CurY, Ypos + CurX, 2*CurX, Horizontal);
     }
     if (D < 0)
     { 
